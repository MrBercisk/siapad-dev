$(document).ready(function () {
	...
	var table = $("#pendapatan").DataTable({
		...
		ajax: {
			...
			dataSrc: function (json) {
				...
				console.log(
					"idstsmasternya dan iddinas yang diambil:",
					json.extra_data
				);
				return json.data;
			},
			...
			$.ajax({
				...
				success: function (response) {
					if (response.success) {
						...
						table.ajax
							.url(
								"PendDaerah/get_datatable_data?id=" +
									recordId +
									"&iddinas=" +
									iddinasId
							)
	...
	$("#pendapatan").on("click", ".edit-data", function (e) {
		...
		if (idstsmaster && iddinas) {
			...
			$.ajax({
				...
				success: function (response) {
					if (response.success) {
						$('#editTableForm input[name="idstsmaster"]').val(
							response.data.idstsmaster
						);
						$('#editTableForm input[name="iduptd"]').val(response.data.iduptd);
						$('#editTableForm input[name="idwp"]').val(response.data.idwp);
						$('#editTableForm input[name="idrapbd"]').val(
							response.data.idrapbd
						);
						$('#editTableForm input[name="nourut"]').val(response.data.nourut);
						$('#editTableForm input[name="nobukti"]').val(
							response.data.nobukti
						);
						$('#editTableForm input[name="tglpajak"]').val(
							response.data.tglpajak
						);
						$('#editTableForm input[name="blnpajak"]').val(
							response.data.blnpajak
						);
						$('#editTableForm input[name="thnpajak"]').val(
							response.data.thnpajak
						);
						$('#editTableForm input[name="jumlah"]').val(response.data.jumlah);
						$('#editTableForm input[name="total"]').val(response.data.total);
						$('#editTableForm input[name="prs_denda"]').val(
							response.data.prs_denda
						);
						$('#editTableForm input[name="nil_denda"]').val(
							response.data.nil_denda
						);
						$('#editTableForm input[name="keterangan"]').val(
							response.data.keterangan
						);
						$('#editTableForm input[name="formulir"]').val(
							response.data.formulir
						);
						$('#editTableForm input[name="kodebayar"]').val(
							response.data.kodebayar
						);
						$('#editTableForm input[name="tgl_input"]').val(
							response.data.tgl_input
						);
						$('#editTableForm input[name="nopelaporan"]').val(
							response.data.nopelaporan
						);
						$('#editTableForm input[name="alamatop"]').val(
							response.data.alamatop
						);
	...
	$("#table-buttons").on("click", "#add-data", function () {
		...
		if (idstsmaster && iddinas) {
			console.log(
				"Tambah data dengan idstsmaster:",
				idstsmaster,
				"dan iddinas:",
				iddinas
			);
