...
class Mpiutanglainnya extends CI_Model {
    ...
    public function formInsert() {
        // Mengambil data tanda tangan dari database
        $ttddata = $this->db
        ...
        // Membuat opsi tanda tangan
        $opsittd = '<option></option>';
        foreach ($ttddata as $ttd) {
            $opsittd .= '<option value="'.$ttd->id.'">'.$ttd->nama.'</option>';
        }
    
        // Mengambil opsi rekening
        $opsiRek = $this->iniopsirekening();
    
        // Membuat form dengan parameter yang telah disesuaikan
        $form[] = '
        <div class="card">
            <div class="card-body">
                <form id="reportForm" action="' . site_url('pembukuanskpd/piutanglainnya/cetak') . '" class="form-row" method="post" onsubmit="printForm(); return false;">
        ...
        xhr.onload = function () {
            ...
            if (xhr.status === 200) {
                if (printWindow && !printWindow.closed) {
                    ...
                    printWindow.document.open();
                    printWindow.document.write("<html><head><title>SIAPAD - Buku Pembantu Piutang Pajak </title>");
