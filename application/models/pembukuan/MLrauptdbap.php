...
class MLrauptdbap extends CI_Model {
    public function ambildata($tglawal, $tglakhir, $tahun) {
        $mysqli = $this->db->conn_id; 
 
        $statment = $mysqli->prepare("CALL spRptLRAUPTDHarian(?, ?, ?)");
        $statment->bind_param('sss', $tglawal, $tglakhir, $tahun);  
    ...
    public function formInsert() {
        ...
        <div class="card">
            <div class="card-body">
                <form id="reportForm" action="' . site_url('pembukuan/Lrauptdbap/cetak') . '" class="form-row" method="post" onsubmit="printForm(); return false;">
        ...
        xhr.onload = function () {
            ...
            if (xhr.status === 200) {
                if (printWindow && !printWindow.closed) {
                    ...
                    printWindow.document.open();
                    printWindow.document.write("<html><head><title>SIAPAD - Lra Harian UPTD Bapenda</title>");
