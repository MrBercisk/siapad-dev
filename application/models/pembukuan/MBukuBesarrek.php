...
class Mbukubesarrek extends CI_Model {
    public function get_apbd_apbdp_total($tahun, $iduptd, $idrekheader) {
        ...
        $this->db->where('idheader', $idrekheader);
        $query = $this->db->get();
        return $query->row();
    }
    public function ambildata($iduptd,$idrekheader,$tahun,$bulan,$bulanakhir) {
        $mysqli = $this->db->conn_id; 
 
        $statment = $mysqli->prepare("CALL spRptBBPPKUPTDRek(?, ?, ?, ?, ?)");
        $statment->bind_param('sssss', $iduptd,$idrekheader,$tahun,$bulan,$bulanakhir);  
    ...
    public function formInsert() {
        ...
        <div class="card">
            <div class="card-body">
                <form id="reportForm" action="' . site_url('pembukuan/bukubesarrek/cetak') . '" class="form-row" method="post" onsubmit="printForm(); return false;">
                            ...
                            <div class="col-md-4">
                                  ...
                                  <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select id="kdrekening" name="idrekheader" class="form-control select2" data-placeholder="Pilih Rekening" style="width: 100%;">
                                                '.$opsiRek.'
                                        </select>
                                    </div>
                                </div>
        ...
        xhr.onload = function () {
            ...
            if (xhr.status === 200) {
                if (printWindow && !printWindow.closed) {
                    ...
                    printWindow.document.open();
                    printWindow.document.write("<html><head><title>SIAPAD - Buku Besar Kas UPTD Bapenda per rekening</title>");
