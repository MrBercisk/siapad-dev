...
<img src="<?= base_url('/assets/img/logo.png') ?>" alt="Logo">
    <h2>DAFTAR PENERBITAN SPTPD <?php if (!empty($kdrekening)) : ?>
                                    <?= strtoupper($kdrekening['nmrekening']); ?>
                                <?php endif; ?> </h2>
    <h3> <?= strtoupper(htmlspecialchars($format_bulan)) ?> <?= htmlspecialchars($format_tahun) ?></h3>
    <h3>SPTPD YANG TERBIT BULAN <?= strtoupper(htmlspecialchars($format_bulan)) ?> <?= htmlspecialchars($format_tahun) ?></h3>
    <h3>(DATA BIDANG PAJAK)</h3>
