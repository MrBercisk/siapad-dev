...
if (!empty($tablenya)): ?>
    ...
    $data_pertama = true;
    $current_nrekening = null; 

    $data_by_nrekening = [];
    foreach ($tablenya as $row) {
        $data_by_nrekening[$row['nmrekening']][] = $row;
    }

    // Mengurutkan nmrekening
    ksort($data_by_nrekening);

    foreach ($data_by_nrekening as $nmrekening => $rows): 
        if (!empty($rows)): 
            ?>
            ...
            foreach ($rows as $row):  
                ...
                if ($data_pertama || (substr($tanggal, -2) == '01' && $bulan_sebelumnya !== $bulan_saat_ini)): 
                    ...
                    <tr>
                        <td></td>
                        <td colspan="7" style="text-align: left; font-weight: bold;"><?= $bulan_saat_ini; ?></td>
                    </tr>
                    <?php 
                    $data_pertama = false;
                endif;
