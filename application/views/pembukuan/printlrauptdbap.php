...
<img src="<?= base_url('/assets/img/logo.png') ?>" alt="Logo">
    ...
    <table border="1">
        ...
        <tr>
            <th colspan="2">s.d. HARI LALU</th>
            <th colspan="2">HARI INI</th>
            <th colspan="3">s.d. HARI INI</th>
        </tr>
        <tr>
            ...
            if (!empty($tablenya)) : 
                ...
                foreach ($kelompokin as $nmuptd => $kelompok_nmrekening) {
                    ...
                    foreach ($kelompok_nmrekening as $nmrekening => $rows) {
                        if (isset($rows[1])) {
                            $total_apbd_nmuptd += $rows[1]['apbd']; 
                            $total_apbdp_nmuptd += $rows[1]['apbdp'];
                        }
                        foreach ($rows as $row) {
                    ...
                    if ($apbdp_checkbox) {
                        $persensampaihariini = $total_apbdp_nmuptd > 0 ? ($totsampaihariini / $totseluruh_apbdp) * 100 : 0;
                        $sisasampaihariini = $totsampaihariini - $totseluruh_apbdp;
                        $persensisasampaihariini =  $total_apbdp_nmuptd > 0 ?  ($sisasampaihariini / $totseluruh_apbdp) * 100 : 0;
                    } else {
                        $persensampaihariini =  $total_apbd_nmuptd > 0 ?  ($totsampaihariini / $totseluruh_apbd) * 100 : 0;
                        $sisasampaihariini = $totsampaihariini - $totseluruh_apbd;
                        $persensisasampaihariini =  $total_apbd_nmuptd > 0 ? ($sisasampaihariini / $totseluruh_apbd) * 100 : 0;
                    }
                    ...
                    $totseluruh_apbd += $total_apbd;
                    /* $totseluruh_apbd += $barisKedua['apbd'];
                    $totseluruh_apbdp += $barisKedua['apbdp']; */
                } ?>
                <tr>
                    <td></td>
