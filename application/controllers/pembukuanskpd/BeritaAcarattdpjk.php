...
class BeritaAcarattdpjk extends CI_Controller {
	public function __construct() {
        parent::__construct();
		$this->load->model('pembukuanskpd/MBattdpjk');
    }
	...
	{	
		...
		$data = [
			...
			'title' => $setpage->title,
			'link' => $setpage->link,
			'forminsert' =>  implode($this->MBattdpjk->formInsert())
	
		];
		$this->load->view('pembukuanskpd/battdpjk',$data);
	}
    ...
    public function cetak() {
        ...
        $tablenya = $this->MBattdpjk->cetaktotal($tahun, $bulan, $kdrekening);
        ...
        ob_start();
        $html = $this->load->view('pembukuanskpd/beritaacarattdpjk', $data, true);
        ob_get_clean();
