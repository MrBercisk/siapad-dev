...
class SptpdTerbit extends CI_Controller {
	public function __construct() {
        parent::__construct();
		$this->load->model('pembukuan/MSptpdterbit');
    }
	...
	{	
		...
		$data = [
			...
			'title' => $setpage->title,
			'link' => $setpage->link,
			'forminsert' =>  implode($this->MSptpdterbit->formInsert())
	
		];
		$this->load->view('pembukuan/sptpdterbit',$data);
	}
    ...
    public function cetak() {
        ...
        $tablenya = $this->MSptpdterbit->cetaktotal($tahun, $bulan, $kdrekening);
        ...
        ob_start();
        $html = $this->load->view('pembukuan/printsptpdterbit', $data, true);
        ob_get_clean();
        ...
        $dompdf->render();
        $dompdf->stream("sptpdterbit.pdf", array("Attachment" => 0));
    }
